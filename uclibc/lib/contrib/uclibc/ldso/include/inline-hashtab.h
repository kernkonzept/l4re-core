...
htab_create (void)
{
  struct hashtab *ht = _dl_malloc (sizeof (struct hashtab));

  if (! ht)
    return NULL;
  ht->size = 3;
  ht->entries = _dl_malloc (sizeof (void *) * ht->size);
  ht->free = free;
  if (! ht->entries)
    {
      if (ht->free)
  ...
  _dl_memset (ht->entries, 0, sizeof (void *) * ht->size);
...
htab_expand (struct hashtab *htab, int (*hash_fn) (void *))
{
  ...
  nentries = _dl_calloc (sizeof (void *), nsize);
  if (nentries == NULL)
    return 0;
